...
def inccost(state,action_mat,tt,c,fb,utc,rc,ns,nl):
    ...
    for i in range(nfb):
        if state[i] > 0:
            z = z + c[nl+int(fb[i])]
    return z
#This function calculates the total cost for a given sample path
def zpath(state,stage,fb,c,tt,rt,utc,nl,ns,nt,sc,rc,i):
    np.random.seed(i)
    z = 0
    states_path = {}
    states_path[stage] = np.copy(state)
    for j in range(stage+1,nt):
            current_state = np.copy(states_path[j-1])
            #opt_action_mat = hueris(current_state,fb,c,tt,rt,utc,nl,ns,rc)
            opt_action_mat = np.matrix(np.ones((2,2))*-1)
    ...
    z = z + inccost(np.copy(states_path[nt-1]), np.matrix(np.ones((2,2))*-1),tt,c,fb,utc,rc,ns,nl)
    return z
#Here we define a function that caculates the estimated cost-to-go       
def costtogo(state,stage,fb,c,tt,rt,utc,nl,ns,nt,sc,itr,rc,ran_seed): 
    seed_list = list(range(ran_seed*itr, (ran_seed+1)*itr))
    costs = []
    for i in seed_list:
        cost_temp = delayed(zpath)(state,stage,fb,c,tt,rt,utc,nl,ns,nt,sc,rc,i)
    ...
    if k>0:   
        ...
        for i in range(len(assigns)):
            ...
            for j in range(len(assign)):
                ...
                out_sc1 = costtogo(state_fut,stage_now+1,fb,c,tt,rt,utc,nl,ns,nt,sc1,iters,rc,ran_seed)
                out_sc2 = costtogo(state_fut,stage_now+1,fb,c,tt,rt,utc,nl,ns,nt,sc2,iters,rc,ran_seed)
                out_sc3 =  costtogo(state_fut,stage_now+1,fb,c,tt,rt,utc,nl,ns,nt,sc3,iters,rc,ran_seed)                
